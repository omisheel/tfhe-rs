use criterion::{criterion_group, criterion_main, Criterion};
use std::time::Duration;
use tfhe::core_crypto::prelude::*;
use tfhe::core_crypto::fft_impl::fft64::crypto::bootstrap::blind_rotate_assign_scratch;

fn simple_blind_rotate_test(c: &mut Criterion) {
    let mut group = c.benchmark_group("blind_rotate_test");
    group.measurement_time(Duration::from_secs(10));
    group.sample_size(10);

    // Parameters for a small test
    let lwe_dimension = LweDimension(512);
    let glwe_dimension = GlweDimension(2);
    let polynomial_size = PolynomialSize(512);
    let decomp_base_log = DecompositionBaseLog(10);
    let decomp_level_count = DecompositionLevelCount(2);
    let std_dev = StandardDev(0.00001);
    let ciphertext_modulus = CiphertextModulus::new_native();

    // Create PRNG
    let mut seeder = new_seeder();
    let seeder = seeder.as_mut();
    let mut encryption_generator = 
        EncryptionRandomGenerator::<DefaultRandomGenerator>::new(seeder.seed(), seeder);
    let mut secret_generator = 
        SecretRandomGenerator::<DefaultRandomGenerator>::new(seeder.seed());

    // Generate keys
    let input_lwe_secret_key = allocate_and_generate_new_binary_lwe_secret_key(
        lwe_dimension,
        &mut secret_generator,
    );
    
    let _output_glwe_secret_key: GlweSecretKeyOwned<u64> =
        allocate_and_generate_new_binary_glwe_secret_key(
            glwe_dimension,
            polynomial_size,
            &mut secret_generator,
        );

    // Create empty bootstrapping key
    let mut fourier_bsk = FourierLweBootstrapKey::new(
        lwe_dimension,
        glwe_dimension.to_glwe_size(),
        polynomial_size,
        decomp_base_log,
        decomp_level_count,
    );

    // Create standard bootstrapping key
    let std_bsk = LweBootstrapKey::new(
        0u64,
        glwe_dimension.to_glwe_size(),
        polynomial_size,
        decomp_base_log,
        decomp_level_count,
        lwe_dimension,
        ciphertext_modulus,
    );

    let fft = Fft::new(polynomial_size);
    let fft_view = fft.as_view();

    // Fill Fourier BSK
    let mut buffers = ComputationBuffers::new();
    buffers.resize(
        fft_view.forward_scratch()
            .unwrap()
            .unaligned_bytes_required(),
    );
    fourier_bsk
        .as_mut_view()
        .fill_with_forward_fourier(std_bsk.as_view(), fft_view, buffers.stack());

    // Create test LWE ciphertext
    let lwe_ct = allocate_and_encrypt_new_lwe_ciphertext(
        &input_lwe_secret_key,
        Plaintext(1u64 << 60),
        DynamicDistribution::new_gaussian_from_std_dev(std_dev),
        ciphertext_modulus,
        &mut encryption_generator,
    );

    // Create lookup table (accumulator)
    let mut accumulator = GlweCiphertext::new(
        0u64,
        glwe_dimension.to_glwe_size(),
        polynomial_size,
        ciphertext_modulus,
    );

    // Initialize with identity LUT
    let delta = 1u64 << (64 - 1 - polynomial_size.log2().0);
    accumulator
        .as_mut_polynomial_list()
        .iter_mut()
        .enumerate()
        .for_each(|(poly_idx, mut poly)| {
            if poly_idx == 0 {
                poly.iter_mut()
                    .enumerate()
                    .for_each(|(coeff_idx, coeff)| {
                        *coeff = (coeff_idx as u64).wrapping_mul(delta);
                    });
            }
        });

    // Modulus switch for blind rotation
    let log_modulus = polynomial_size.to_blind_rotation_input_modulus_log();
    let msed_lwe = lwe_ciphertext_modulus_switch(lwe_ct.as_view(), log_modulus);

    // Setup buffers for blind rotation
    let mut br_buffers = ComputationBuffers::new();
    br_buffers.resize(
        blind_rotate_assign_scratch::<u64>(
            glwe_dimension.to_glwe_size(),
            polynomial_size,
            fft_view,
        )
        .unwrap()
        .unaligned_bytes_required(),
    );

    group.bench_function("blind_rotate_assign_u64", |b| {
        b.iter(|| {
            let mut lut_copy = accumulator.clone();
            fourier_bsk.as_view().blind_rotate_assign(
                lut_copy.as_mut_view(),
                &msed_lwe,
                fft_view,
                br_buffers.stack(),
            );
            criterion::black_box(&lut_copy);
        });
    });

    group.finish();
}

criterion_group!(blind_rotate_test_group, simple_blind_rotate_test);
criterion_main!(blind_rotate_test_group);